...
% Slide 18: Continuous Integration Pipeline
\begin{frame}{CI Pipeline (GitHub Actions)}
    Triggered on every PR and push to \texttt{main}:
    \vspace{0.3cm}
    \begin{enumerate}
        \item Installs dependencies (\texttt{pnpm}) \& checks types.
        \item Runs Backend Unit \& E2E tests (Vitest + MongoDB-Memory-Server).
        \item Runs Dashboard Component \& Browser tests (Vitest + Playwright cross-browser).
        \item Builds \& tests Go Client tests.
        \item Merges coverage data into unifying Cobertura artifacts.
    \end{enumerate}
\end{frame}
...
\begin{frame}{Release \& Deployment Validation}
    \begin{itemize}
        ...
        \item \textbf{Observability:} Deployment smoke-tests verify behavior via \texttt{GET /health} endpoint and Database index validation scripts.
    \end{itemize}
\end{frame}
